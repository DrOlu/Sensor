...
<body>
  <div class="shell">
    <div class="header">
      ...
      <div>
        <div class="brand">Sensor</div>
      </div>
    </div>
    <h1 class="title-row"><span>${safeTitle}</span>${titleIcon}</h1>
    <p class="subtitle">${safeMessage}</p>
    ${detailBlock}
    <div class="footer">You can close this window and return to <span class="accent">Sensor</span>.</div>
  </div>
  ${closeScript}
</body>
</html>`;
  ...
  if (!session.resolve || !session.reject) {
    res.end(
      renderOAuthPage({
        ...
        message: "This sign-in request is not active yet. Return to Sensor and try again.",
        status: "warning",
      })
    );
    return;
