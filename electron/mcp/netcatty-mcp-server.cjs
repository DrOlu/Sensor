...
server.tool(
  "get_environment",
  "Get information about the current Sensor scope: all terminal sessions exposed by Sensor, their session IDs, OS, shell hints, and connection status. Sessions may be remote hosts, a local terminal, Mosh-backed shells, or serial port connections (network devices, embedded systems). Serial sessions have protocol 'serial' and shellType 'raw'. SSH sessions with deviceType 'network' are network equipment (Huawei VRP, Cisco IOS, etc.) using vendor CLIs instead of a standard shell. Call this first before executing commands.",
  {},
  async () => {
...
server.tool(
  "list_attachments",
  "List local files explicitly attached by the user in this Sensor AI chat. Use this before read_attachment when the user asks about a pasted, dropped, or uploaded local file.",
  {},
  async () => {
...
server.tool(
  "read_attachment",
  "Read a local file explicitly attached by the user in this Sensor AI chat. This tool only accepts file paths or filenames that were registered from user attachments; it cannot read arbitrary local files.",
  {
...
server.tool(
  "terminal_start",
  "Start a long-running command on a Sensor terminal session without waiting for final completion. The command still runs in the visible terminal/PTTY so the user can watch live output. Prefer this whenever the command may exceed about 2 minutes, or when it streams output for an extended period, such as builds, scans, watch commands, and log-follow commands. After starting, wait at least about 30 seconds before the first terminal_poll unless you have a strong reason to check sooner.",
  {
    sessionId: z.string().describe("The terminal session ID (from get_environment) to execute on."),
...
server.tool(
  "terminal_poll",
  "Poll a long-running Sensor command that was started with terminal_start. Returns incremental output since the given offset and the current status. Use the returned nextOffset for the next poll. If outputTruncated is true, only the retained tail starting at outputBaseOffset is still available. Do not poll aggressively: wait at least about 30 seconds between polls unless the tool output explicitly justifies checking sooner. As soon as completed is true, stop polling and analyze the final result immediately.",
  {
...
server.tool(
  "terminal_stop",
  "Stop a long-running Sensor command that was started with terminal_start. This sends Ctrl+C to the running terminal job and returns its latest state.",
  {
    jobId: z.string().describe("The background job ID returned by terminal_start."),
  },
  async ({ jobId }) => {
