...
export const enAiMessages: Messages = {
  ...
  'ai.providers.contextWindow.help': 'Leave blank to use the model list value when available, otherwise Sensor uses a safe default.',
  ...
  'ai.cursor.apiKeyEnvHint': 'Cursor can use CURSOR_API_KEY from your shell. Save a key here only if you want Sensor to override it.',
  'ai.cursor.apiKeyOverrideHint': 'Sensor will use the saved key here before CURSOR_API_KEY.',
  ...
  'ai.codebuddy.persistSession.hint': "Keep CodeBuddy's own session transcript on disk so CodeBuddy can resume it later. Turn off to disable CodeBuddy session storage; Sensor may still save chat history and external session metadata. File checkpointing is skipped. Requires CodeBuddy CLI 2.125.1 or newer.",
  ...
  'ai.grok.runtime.acp.description':
    'Talk to Grok over Agent Client Protocol (grok agent stdio). Injects Sensor MCP on session/new. Turn off to use the original headless streaming-json CLI path.',
  ...
  'ai.toolAccess.description': 'Choose how external agents access Sensor sessions. MCP exposes the built-in server, while Skills + CLI points agents to the local Sensor skill and CLI commands.',
  'ai.toolAccess.mode.mcp': 'MCP',
  'ai.toolAccess.mode.skills': 'Skills + CLI',
  'ai.toolAccess.mcpPrompt.title': 'Prompt for your AI client',
  'ai.toolAccess.mcpPrompt.description': 'Paste this prompt into your AI client (Codex, Claude Code, …) and it will register Sensor MCP for you.',
  ...
  'ai.toolAccess.skills.description': 'In Skills + CLI mode, agents are pointed to this local skill file automatically. The Sensor CLI launcher path is provided to the agent in each session.',
  ...
  'ai.externalMcp.description': 'Expose Sensor as an MCP server for external clients such as Codex, Claude Code, Cursor, and Grok. Uses the same catalog tools as in-app agents (terminal, SFTP, Vault, port forwarding). Keep Sensor running while clients are connected.',
  ...
  'ai.externalMcp.mode.description': 'Temporary mode auto-disables after idle timeout. Always-on restores External MCP when Sensor starts.',
  ...
  'ai.externalMcp.usage.keepRunning': '1. Turn on External MCP and keep Sensor running.',
  ...
  'ai.externalMcp.permissionMode.hint': 'Same setting as Settings → AI → Safety. Auto runs Sensor write tools without Sensor approval prompts; Confirm asks each time. External clients (Codex / Claude / Grok) may still show their own tool approval UI.',
  ...
  'ai.externalMcp.codexAdded': 'Codex MCP entry and Sensor skill added. Restart Codex or open a new Codex session.',
  'ai.externalMcp.claudeAdded': 'Claude Code MCP entry and Sensor skill added. Restart Claude Code or open a new Claude Code session.',
  'ai.externalMcp.grokAdded': 'Grok MCP entry and Sensor skill added. Restart Grok or open a new Grok session.',
  ...
  'ai.safety.permissionMode.description': 'Controls how the AI interacts with your Sensor terminal sessions. Observer mode blocks write operations that go through Sensor. External agent CLIs may still have their own local tools and approval flow.',
  ...
  'ai.safety.commandTimeout.description': 'Maximum seconds a command can run before being terminated through Sensor execution.',
  ...
  'ai.safety.blocklist.description': 'Regex patterns to block dangerous commands executed through Sensor.',
  ...
  'ai.safety.note': 'These safety settings are enforced for actions that go through Sensor. External agent CLIs may also expose local tools that are governed by the agent itself.',
