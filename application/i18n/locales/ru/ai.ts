...
export const ruAiMessages: Messages = {
  ...
  'ai.providers.contextWindow.help': 'Оставьте пустым, чтобы использовать значение из списка моделей, если оно доступно; иначе Sensor применит безопасное значение по умолчанию.',
  ...
  'ai.cursor.apiKeyEnvHint': 'Cursor может использовать CURSOR_API_KEY из shell. Сохраняйте ключ здесь только если хотите переопределить его в Sensor.',
  'ai.cursor.apiKeyOverrideHint': 'Sensor сначала использует сохранённый здесь ключ, затем CURSOR_API_KEY.',
  ...
  'ai.codebuddy.persistSession.hint': 'Сохраняет на диск собственную историю сессии CodeBuddy, чтобы CodeBuddy мог возобновить её позже. Отключите, чтобы запретить хранение сессии CodeBuddy; Sensor всё ещё может сохранять историю чата и метаданные внешней сессии. Контрольные точки файлов пропускаются. Требуется CodeBuddy CLI 2.125.1 или новее.',
  ...
  'ai.grok.runtime.acp.description':
    'Подключение к Grok через Agent Client Protocol (grok agent stdio). Sensor MCP внедряется в session/new. Выключите, чтобы использовать исходный headless streaming-json CLI.',
  ...
  'ai.toolAccess.description': 'Выберите, как внешние агенты получают доступ к сессиям Sensor. MCP предоставляет встроенный сервер, а Skills + CLI указывает агентам на локальный skill Sensor и команды CLI.',
  'ai.toolAccess.mode.mcp': 'MCP',
  'ai.toolAccess.mode.skills': 'Skills + CLI',
  'ai.toolAccess.mcpPrompt.title': 'Промпт для вашего ИИ-клиента',
  'ai.toolAccess.mcpPrompt.description': 'Вставьте этот промпт в ИИ-клиент (Codex, Claude Code и т. д.), и он сам зарегистрирует MCP Sensor.',
  ...
  'ai.toolAccess.skills.description': 'В режиме Skills + CLI агенты автоматически получают путь к этому локальному skill-файлу. Путь к лаунчеру Sensor CLI передаётся агенту в каждой сессии.',
  'ai.toolAccess.skills.unavailable': 'Путь к skill-файлу недоступен',

  'ai.externalMcp.title': 'Внешний MCP',
  'ai.externalMcp.description': 'Откройте Sensor как MCP-сервер для Codex, Claude Code, Cursor и Grok. Набор инструментов тот же, что у встроенных агентов. Держите Sensor запущенным, пока клиенты подключены.',
  ...
  'ai.externalMcp.mode.description': 'Временный режим отключается после простоя. Постоянный восстанавливает External MCP при запуске Sensor.',
  ...
  'ai.externalMcp.usage.keepRunning': '1. Включите External MCP и держите Sensor запущенным.',
  ...
  'ai.externalMcp.permissionMode.hint': 'Тот же параметр, что Settings → AI → Safety. Auto выполняет write-инструменты Sensor без запросов; Confirm спрашивает каждый раз. Внешние клиенты (Codex / Claude / Grok) могут показывать своё подтверждение инструментов.',
  ...
  'ai.externalMcp.codexAdded': 'Запись MCP для Codex и навык Sensor добавлены. Перезапустите Codex или откройте новую сессию.',
  'ai.externalMcp.claudeAdded': 'Запись MCP для Claude Code и навык Sensor добавлены. Перезапустите Claude Code или откройте новую сессию.',
  'ai.externalMcp.grokAdded': 'Запись MCP для Grok и навык Sensor добавлены. Перезапустите Grok или откройте новую сессию.',
  ...
  'ai.safety.permissionMode.description': 'Управляет тем, как AI взаимодействует с вашими терминалами. Режим наблюдателя блокирует все операции записи через Sensor и применяется как к встроенным, так и к внешним агентам. Режим подтверждения носит рекомендательный характер для внешних агентов (они управляют собственным потоком одобрения инструментов).',
  ...
  'ai.safety.blocklist.description': 'Regex-шаблоны для блокировки опасных команд. Применяется как к встроенным, так и к внешним агентам через механизм выполнения Sensor.',
  ...
  'ai.safety.note': 'Эти настройки безопасности применяются к действиям, выполняемым через Sensor. Внешние CLI-агенты могут иметь собственные локальные инструменты и собственные правила управления ими.',
