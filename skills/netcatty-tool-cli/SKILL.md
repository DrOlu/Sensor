---
name: sensor-tool-cli
description: Use this skill when an external agent needs to operate on Sensor sessions through Skills + CLI instead of the sensor-remote-hosts MCP server.
---

# Sensor Tool CLI

Use this skill for external ACP agents when Sensor is configured for `Skills + CLI` mode.

For routine tasks, the host prompt is usually enough. Read only the reference that matches the task type.

## Router

1. Use the exact Sensor CLI prefix provided by the host prompt.
2. Keep `--chat-session <chat-session-id>` on every Sensor CLI call. Do not omit it.
...
- Run Sensor CLI commands strictly serially.
- Treat Sensor CLI errors as authoritative.
- Never ask the user for SSH credentials, key paths, proxy settings, or jump-host details when Sensor session access already exists.
...
- When the execution surface accepts argv-style calls, use the Sensor launcher path as the executable and pass subcommands and flags as separate arguments instead of wrapping it in another shell.
