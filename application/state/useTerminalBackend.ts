...
type PluginConnectionStartOptions = SensorPluginConnectionStartRequest & {
  signal?: AbortSignal;
};
...
export async function startPluginConnectionWithBridge(
  bridge: Pick<SensorBridge, "invokePluginExtensionProvider" | "startPluginConnection">,
...
export async function signalPluginConnectionWithBridge(
  bridge: Pick<SensorBridge, "controlPluginConnection">,
  sessionId: string,
...
export const useTerminalBackend = () => {
  ...
  const interruptSession = useCallback((...args: Parameters<NonNullable<SensorBridge["interruptSession"]>>) => {
  ...
  const onSessionData = useCallback((
    sessionId: string,
    cb: Parameters<SensorBridge["onSessionData"]>[1],
