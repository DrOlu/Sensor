...
`exec` calls are internal agent transport calls. Always include `--session <session-id>`. The current chat session is already bound in the host environment.
...
1. If the host prompt already gives a connected default target session, prefer it directly:
   - `<sensor-cli-prefix> session --session <default-session-id> --json`
   - `<sensor-cli-prefix> exec --session <default-session-id> --json -- <command>`
2. Otherwise:
   - `<sensor-cli-prefix> env --json`
   - Choose a `connected` session.
   - `<sensor-cli-prefix> session --session <session-id> --json`
   - `<sensor-cli-prefix> exec --session <session-id> --json -- <command>`
...
- Long-running flow:
  - `<sensor-cli-prefix> job-start --session <session-id> --json -- <command>`
  - wait before polling unless the output clearly justifies checking sooner
  - `<sensor-cli-prefix> job-poll --job <job-id> --offset <offset> --json`
  - if the user asks to stop it: `<sensor-cli-prefix> job-stop --job <job-id> --json`
