...
When "Restore previous terminal tabs and workspace layout" is enabled, Sensor restores the prior terminal workspace on launch. Restored terminals are marked with `restoreState: "restored-disconnected"` and stay disconnected until the user explicitly reconnects them.
