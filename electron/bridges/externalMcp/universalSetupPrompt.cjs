...
function buildUniversalSetupPrompt({ launcherPath, discoveryPath, skillContent }) {
  if (typeof launcherPath !== "string" || !launcherPath.trim()) {
    throw new Error("Sensor External MCP launcher path is unavailable.");
  }
  if (typeof discoveryPath !== "string" || !discoveryPath.trim()) {
    throw new Error("Sensor External MCP discovery path is unavailable.");
  }
  if (typeof skillContent !== "string" || !skillContent.trim()) {
    throw new Error("Sensor MCP skill content is unavailable.");
  }
  ...
  return `Install the Sensor integration in the AI coding client you are currently running in.
...
2. If an entry named \`${EXTERNAL_MCP_NAME}\` already exists and is clearly an older Sensor entry, update it. If it points to an unrelated command, stop and report the conflict instead of overwriting it.
3. If the client supports Agent Skills, install the supplied skill as \`netcatty-mcp/SKILL.md\` in the client's native user-level skills directory. Preserve an existing same-name skill unless it contains \`managed-by: netcatty\`; Sensor-managed copies may be updated.
4. If Agent Skills are unsupported, add the behavioral instructions from the supplied skill to the client's user-level persistent instructions. Keep existing instructions intact and use clearly marked Sensor boundaries. If no user-level persistent instruction mechanism exists, configure MCP only and report that limitation.
