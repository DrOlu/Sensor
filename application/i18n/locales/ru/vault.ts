...
export const ruVaultMessages: Messages = {
  ...
  'vault.import.progress.keepOpen': 'Не закрывайте Sensor, пока импорт выполняется в фоне.',
  ...
  'settings.sshDeepLink.enableDesc': 'Разрешить Sensor обрабатывать ssh:// и telnet:// ссылки из браузеров и других приложений, а также запуски в стиле PuTTY, например -ssh user@host -P 22 -pw password.',
  'settings.explorerContextMenu.title': 'Проводник Windows',
  'settings.explorerContextMenu.enable': 'Показывать «Open in Sensor» в контекстном меню папок',
  ...
  'hostDetails.algorithms.inheritedNotice': 'В текущей группе заданы переопределения алгоритмов для: {categories}. Кнопка «Сбросить» здесь возвращает к спискам группы, а не к значениям Sensor по умолчанию. Чтобы игнорировать ограничение группы, очистите переопределение в настройках алгоритмов группы.',
