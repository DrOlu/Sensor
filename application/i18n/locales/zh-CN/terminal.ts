...
export const zhCNTerminalMessages: Messages = {
  ...
  'settings.sshDeepLink.enable': '用 Sensor 打开 ssh:// 和 telnet:// 链接',
  'settings.sshDeepLink.enableDesc': '允许 Sensor 接管来自浏览器和其他应用的 ssh://、telnet:// 链接，以及堡垒机常用的 PuTTY 启动参数（例如 -ssh user@host -P 22 -pw password）。',
  'settings.explorerContextMenu.title': 'Windows 资源管理器',
  'settings.explorerContextMenu.enable': '在文件夹右键菜单中显示 “Open in Sensor”',
  ...
  'settings.jmsDeepLink.enableDesc': '将 Sensor 注册为 JumpServer jms:// 客户端链接的处理程序，可能与官方 JumpServer 客户端冲突。',
  ...
  'settings.terminal.font.cjk.unavailableWarning': '本机没有安装这个字体。Sensor 会保留设置，并使用安全字体继续显示。',
  ...
  'snippets.import.toast.invalidDesc': '这不是有效的 Sensor 代码片段文件。',
  ...
  'snippets.import.modal.desc': '选择一个或多个 Sensor 代码片段 JSON 文件。导入前会先解析并展示结果，不会直接写入。',
  ...
  'serial.encoding.pendingInput': '请先提交或清空当前输入，再切换编码。',
  'serial.field.byteOrientedBackspace': '按字节退格',
  'serial.field.byteOrientedBackspaceDesc': '发送与字符字节数相同的 DEL，使设备删除整个多字节字符（适用于按字节删除的设备）。若为逐字符删除的行编辑器（如 Linux readline），请关闭。',
