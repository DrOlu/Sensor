...
function buildCompactContext(
  ...
  const contentLines = [
    "[Compact prior Sensor UI context]",
    "The external ACP agent may already have its own persisted session context. Use this compact Sensor UI context only as fallback/background, and prefer the current user request when there is any conflict.",
  ];
