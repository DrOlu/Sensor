...
async function buildUserSkillsContext(electronApp, prompt, selectedSkillSlugs = []) {
  ...
  const parts = [
    "User-managed skills are installed in Sensor.",
  ...
  if (additionalExplicitCount > 0) {
    parts.push(
      `The user selected ${additionalExplicitCount} additional Sensor user skills that were omitted to stay within the prompt budget.`,
    );
  }

  if (unavailableExplicitSlugs.length > 0) {
    parts.push(
      `The user explicitly selected these Sensor user skills for this request, but their content is currently unavailable: ${summarizeSkillSlugs(unavailableExplicitSlugs)}.`,
    );
  }
