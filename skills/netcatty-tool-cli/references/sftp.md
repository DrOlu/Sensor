...
- List a directory:
  - `<sensor-cli-prefix> sftp list --session <session-id> --remote-path <remote-path> --json`
- Read a file:
  - `<sensor-cli-prefix> sftp read --session <session-id> --remote-path <remote-path> --json`
- Write a small text file with known content:
  - `<sensor-cli-prefix> sftp write --session <session-id> --remote-path <remote-path> --content <text> --json`
- Download a remote file to an existing local path:
  - `<sensor-cli-prefix> sftp download --session <session-id> --remote-path <remote-path> --local-path <local-path> --json`
- Upload an existing local file:
  - `<sensor-cli-prefix> sftp upload --session <session-id> --local-path <local-path> --remote-path <remote-path> --json`
- Delete a remote path:
  - `<sensor-cli-prefix> sftp delete --session <session-id> --remote-path <remote-path> --json`
