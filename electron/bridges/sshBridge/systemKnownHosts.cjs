 ...
 * `~/.ssh/known_hosts`). That file — not Sensor's in-app known-hosts vault —
 ...
 * trusted. Sensor's vault snapshot does not get updated when OpenSSH accepts
 ...
 * appear unless the user manually imports/scans the host into Sensor).
 *
