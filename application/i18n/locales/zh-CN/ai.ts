...
export const zhCNAiMessages: Messages = {
  ...
  'ai.providers.contextWindow.help': '留空时优先使用模型列表返回的值；如果没有，Sensor 会使用安全默认值。',
  ...
  'ai.codebuddy.persistSession.hint': '将 CodeBuddy 自身的会话记录保存到磁盘，以便 CodeBuddy 后续恢复会话。关闭后将停用 CodeBuddy 的会话存储；Sensor 仍可能保存聊天记录和外部会话元数据。文件检查点会跳过。需要 CodeBuddy CLI 2.125.1 或更高版本。',
  ...
  'ai.grok.runtime.acp.description':
    '通过 Agent Client Protocol（grok agent stdio）接入 Grok。在 session/new 注入 Sensor MCP。关闭后使用原始 headless streaming-json CLI 路径。',
  ...
  'ai.toolAccess.mode': 'Sensor 接入模式',
  'ai.toolAccess.description': '选择外部 Agent 访问 Sensor 会话的方式。MCP 会暴露内置服务器，Skills + CLI 会引导 Agent 读取本地 Skill 并调用 Sensor CLI。',
  'ai.toolAccess.mode.mcp': 'MCP',
  'ai.toolAccess.mode.skills': 'Skills + CLI',
  'ai.toolAccess.mcpPrompt.title': '喂给 AI 的接入提示词',
  'ai.toolAccess.mcpPrompt.description': '把这段提示词粘贴到你的 AI 客户端（Codex、Claude Code 等），它就会帮你完成 Sensor MCP 的注册。',
  'ai.toolAccess.mcpPrompt.enableHint': '先在下方打开对外 MCP 开关，提示词中才会包含 launcher 路径。',
  'ai.toolAccess.skills.file': 'Skill 文件',
  'ai.toolAccess.skills.description': 'Skills + CLI 模式下，Agent 会自动被指向这个本地 Skill 文件；Sensor CLI 启动路径会在每个会话中提供给 Agent。',
  'ai.toolAccess.skills.unavailable': '暂时无法获取 Skill 文件路径',

  // External MCP
  'ai.externalMcp.title': '对外 MCP',
  'ai.externalMcp.description': '把 Sensor 作为 MCP 服务器暴露给 Codex、Claude Code、Cursor、Grok 等外部客户端。工具面与应用内 Agent 相同（终端、SFTP、Vault、端口转发）。客户端连接期间请保持 Sensor 运行。',
  ...
  'ai.externalMcp.mode.description': '临时模式会在空闲超时后自动关闭；常开模式会在 Sensor 启动时恢复对外 MCP。',
  ...
  'ai.externalMcp.usage.keepRunning': '1. 打开对外 MCP 开关，并保持 Sensor 运行。',
  ...
  'ai.externalMcp.permissionMode.hint': '与「设置 → AI → 安全」为同一项。自动：Sensor 写操作不再弹本应用审批；确认：每次询问。外部客户端（Codex / Claude / Grok）仍可能有自己的工具审批界面。',
  ...
  'ai.externalMcp.codexAdded': '已添加 Codex MCP 条目和 Sensor Skill。请重启 Codex 或打开新会话。',
  'ai.externalMcp.claudeAdded': '已添加 Claude Code MCP 条目和 Sensor Skill。请重启 Claude Code 或打开新会话。',
  'ai.externalMcp.grokAdded': '已添加 Grok MCP 条目和 Sensor Skill。请重启 Grok 或打开新会话。',
  ...
  'ai.userSkills.description': '打开 Sensor 的 Skills 文件夹以添加你自己的技能目录。Sensor 会自动扫描这些 skills，默认只注入轻量索引，只有在请求明显命中某个 skill 时才展开正文。',
  ...
  // AI Error
  'ai.codex.bridgeError': 'Codex 主进程处理器尚未加载。请完全重启 Sensor 或重启 Electron 开发进程，然后重试。',
  ...
  'ai.safety.permissionMode.description': '控制 AI 通过 Sensor 访问终端会话的方式。观察者模式会阻止经由 Sensor 的写操作；外部 Agent CLI 可能仍有自己的本机工具和审批流程。',
  ...
  'ai.safety.commandTimeout.description': '通过 Sensor 执行命令时允许运行的最长秒数，超时将被终止。',
  ...
  'ai.safety.blocklist.description': '用于拦截通过 Sensor 执行的危险命令的正则表达式。',
  ...
  'ai.safety.note': '这些安全设置会约束经由 Sensor 执行的操作。外部 Agent CLI 也可能提供本机工具，那部分由 Agent 自己的控制规则约束。',
