...
- Sensor: `e6ffbd3f6894c810d148002670f5187bf654a345`
- Tabby: `14e2d60b9b6dee84a53c37f05eefeb803787de04`
- Electerm: `6fbddfe55c66bffcb5aaad23676c0dd006e16367`

## Conclusion

[Issue #2206](https://github.com/DrOlu/Sensor/issues/2206) is supportable without a new persistence model or a native font-scanning dependency. Sensor already asks Chromium for every installed font family, uses that result to populate the main terminal-font picker, and persists the CJK fallback as a family-name string. The actual gap is narrower: the separate “Chinese / CJK font” picker is built from eight hard-coded choices, so an installed Chinese font outside that list is never offered.

Tabby is the best product reference. It keeps a simple “main font + fallback font” model, gives both fields local-font autocomplete while retaining free text, and constructs one ordered CSS font stack. Electerm proves that a more flexible ordered list also works, but its tag-list UX is harder to explain and easier to misconfigure. Sensor should keep its existing two-field model and make the CJK picker searchable over installed families, rather than replacing it with an arbitrary font-chain editor.

## Current Sensor behavior and precise gap

- Sensor calls `queryLocalFonts()` once, caches all installed family names, and separately derives likely monospace families for the main font list ([local-font query](https://github.com/DrOlu/Sensor/blob/e6ffbd3f6894c810d148002670f5187bf654a345/lib/localFonts.ts#L90-L164), [store integration](https://github.com/DrOlu/Sensor/blob/e6ffbd3f6894c810d148002670f5187bf654a345/application/state/fontStore.ts#L60-L95)). Electron explicitly grants the app origin the `local-fonts` permission ([permission policy](https://github.com/DrOlu/Sensor/blob/e6ffbd3f6894c810d148002670f5187bf654a345/electron/main.cjs#L955-L1004)).
- The main terminal-font picker already receives those dynamically discovered fonts. The CJK picker does not: it owns a static eight-entry `OPTIONS` array and only filters those entries by installation status ([CJK picker](https://github.com/DrOlu/Sensor/blob/e6ffbd3f6894c810d148002670f5187bf654a345/components/settings/TerminalCjkFontSelect.tsx#L16-L97)). This explains why a locally installed Chinese font can exist but remain unselectable.
- The selected CJK family is already stored as `terminalSettings.fallbackFont`, included in sync, and inserted before Sensor's broader CJK/Nerd/system fallback stack ([settings UI](https://github.com/DrOlu/Sensor/blob/e6ffbd3f6894c810d148002670f5187bf654a345/components/settings/tabs/SettingsTerminalTab.tsx#L511-L522), [sync field list](https://github.com/DrOlu/Sensor/blob/e6ffbd3f6894c810d148002670f5187bf654a345/application/syncPayload.ts#L197-L205), [font-stack composition](https://github.com/DrOlu/Sensor/blob/e6ffbd3f6894c810d148002670f5187bf654a345/infrastructure/config/cjkFonts.ts#L120-L181)). Therefore the first version needs a better picker and source data, not a schema migration.
...
## Recommended Sensor plan

1. Reuse the already-cached full installed-family set in `TerminalCjkFontSelect`; do not make a second `queryLocalFonts()` call.
2. Replace the closed static dropdown with a searchable combobox. Put `Auto` and Sensor's known-safe monospaced CJK choices first, then an “Installed fonts” section. Show each family name using itself and preview a short mixed sample such as `ABC 你好 123`.
3. Retain manual entry, as Tabby and Electerm do. Local-font permission can be unavailable, native lists can be incomplete, and a synced family may not exist on the current device. Manual entry must not suppress Sensor's final fallback stack.
...
- Font installed or removed while Sensor is open: a refresh/retry action should update the list without requiring an application restart.
...
The first implementation should be global-only and should not add Electerm-style arbitrary ordered chains or per-host CJK overrides. Sensor already has a clear global main-font/CJK-fallback relationship; expanding configuration scope before the picker works would add migration, inheritance, and sync complexity without improving #2206's core outcome.
