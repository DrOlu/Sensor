...
function approveSensorCliShellOnly(request) {
  if (request?.kind === "shell") {
    ...
    return {
      kind: "reject",
      feedback:
        "Only Sensor CLI shell commands are allowed. Invoke the netcatty-tool-cli launcher or script prefix provided by the host. Do not pass --chat-session or override NETCATTY_CLI_CHAT_SESSION_ID; the host already bound this process.",
    };
  }
  return {
    kind: "reject",
