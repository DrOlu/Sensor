# Sensor Privacy Policy

Effective date: July 27, 2026

This policy applies to the Sensor desktop application distributed through
the official Sensor GitHub repository. It does not cover third-party
websites, services, plugins, command-line agents, or servers that you choose to
use with Sensor.

## Summary

Sensor does not operate a centralized user account, advertising, analytics,
or crash-reporting service. The Sensor maintainers do not automatically
receive your saved hosts, credentials, terminal contents, files, chat history,
or diagnostic logs.

Sensor stores application data on your device by default. It connects to
...
Depending on the features you use, Sensor may store:
...
You can remove local Sensor data by deleting it in the application or by
uninstalling Sensor and removing its application-data directory. Data kept
by a third-party service must be removed through that service.

## Network connections and third parties

Sensor sends data directly to third parties in the following situations:

- **Updates:** Sensor may contact GitHub to check for and download official
...
- **Cloud sync:** If you enable sync, Sensor sends an encrypted vault to the
...
These services are not operated by the Sensor maintainers. Review their
privacy policies and terms before enabling the corresponding feature.

## Diagnostics and support

Sensor does not automatically upload application logs or crash reports to
...
Material changes to this policy will be published in the Sensor repository.
