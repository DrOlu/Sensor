...
const MODEL_DESCRIPTION_HINTS = Object.freeze({
  ...
  "vault.host.open":
    "Opens a terminal tab for a saved vault host (same as clicking the host in Sensor). Connection may still be establishing when the tool returns — use get_environment or wait briefly before terminal_execute if needed. Auth prompts (passphrase / keyboard-interactive) still require the user in the Sensor UI.",
  "vault.host.import":
