...
test("Add to Codex installs both the MCP entry and Sensor skill", async () => {
  await withSetup({}, async ({ setup, calls, homeDir }) => {
    ...
    assert.match(await fs.readFile(getUserSensorSkillPath("codex", { homeDir }), "utf8"), /name: netcatty-mcp/);
  });
});
...
test("Add to Codex preserves an unmanaged skill and reports the conflict", async () => {
  ...
  await withSetup({
    ...
    prepareHome: async (homeDir) => {
      const skillPath = getUserSensorSkillPath("codex", { homeDir });
    ...
    assert.equal(calls.some(args => args[1] === "add" || args[1] === "remove"), false);
    assert.equal(await fs.readFile(getUserSensorSkillPath("codex", { homeDir }), "utf8"), customContent);
  });
});
