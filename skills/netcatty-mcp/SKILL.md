---
name: netcatty-mcp
description: Use when the user asks to inspect or operate Sensor terminals, remote hosts, SSH or other live sessions, SFTP files, vault data, snippets, scripts, or port forwards through Sensor MCP.
metadata:
  managed-by: netcatty
---
...
Use the Sensor MCP tools whenever the task targets Sensor or a remote session managed by Sensor.
...
- Never use the local shell for commands intended for a Sensor session.
...
- Treat Sensor permission denials and tool errors as authoritative. Do not bypass them with local commands or alternate connection tools.
