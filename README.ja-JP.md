...
[Scoop](https://scoop.sh/) をインストール済みの場合、コミュニティが管理する [lemon バケット](https://github.com/hoilc/scoop-lemon/blob/master/bucket/sensor.json) から Sensor をインストールできます。このバケットは Sensor の公式 GitHub Releases からインストーラーをダウンロードします：
...
更新またはアンインストールの前に Sensor を終了してください：
...
これはサードパーティ製パッケージであり、Sensor の公式バケットではありません。更新は GitHub Releases より遅れる場合があります。現在は x64 のみに対応しています。設定は `%APPDATA%\sensor` に保存され、更新や通常のアンインストール後も残ります。`scoop uninstall sensor --purge` はこれらの設定も削除します。この方法でインストールした場合は、Scoop で更新を管理してください。
