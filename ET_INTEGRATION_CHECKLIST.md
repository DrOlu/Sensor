...
> （`moshcatty-*` releases）；Sensor 内已无 Cygwin 构建流水线 / FluentTerminal 回退。
...
- **构建系统**：Mosh 客户端在 **MoshCatty** 仓库用 Rust 构建并发布；Sensor 只
...
| env `MOSH_BIN_RELEASE` / 仓库 `MoshCatty` / tag `moshcatty-*` | env `ET_BIN_RELEASE` / 仓库 `Sensor-et-bin` / tag `et-bin-*` |
...
- [x] **1.12** `.github/workflows/build-et-binaries.yml` —— 四个构建 job + 一个
      `release` job（dispatch 且 `release_tag` 非空时发布到 `Sensor-et-bin`，附
