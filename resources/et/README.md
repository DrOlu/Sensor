...
with the Sensor installer. Sensor launches this bundled `et` directly
...
2. When manually dispatched with `release_tag`, that workflow publishes the
   binaries to the dedicated `DrOlu/Sensor-et-bin` repository. The
...
3. Release packaging runs `scripts/resolve-et-bin-release.cjs` before
   ...
   `et` into this gitignored directory. Sensor does not fall back to a
...
- Sensor is **GPL-3.0**; Apache-2.0 is one-way compatible with GPL-3.0, so
