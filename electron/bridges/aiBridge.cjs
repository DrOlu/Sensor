...
function buildExternalAgentSystemContext({ mode, chatSessionId, defaultTargetSession, userSkillsContext }) {
  ...
  if (mode === "skills") {
    ...
    const scopeHint = chatSessionId
      ? `The current Sensor chat session is already bound in this process via the host environment. Keep using \`--session <session-id>\` to select a terminal. `
      : "";
    ...
    return (
      ...
      `Use the local shell only to invoke Sensor CLI commands. Do not use local shell or filesystem tools for unrelated local-machine work. ` +
      ...
      `If the user stops the run or asks to abort outstanding Sensor work, use \`${cliCommandPrefix} cancel --json\`, and use \`resume\` to re-enable execs for that scope if needed. ` +
